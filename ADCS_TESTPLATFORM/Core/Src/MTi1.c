...
#define IMU_BID			0xff
//MID definitions
#define IMU_GOTO_CONFIG_MID			0x10
#define IMU_GOTO_CONFIG_ACK_MID		0x11
...
uint8_t initIMUConfig(UART_HandleTypeDef* IMUhandle){
    ...
    for(uint32_t retry=0;retry<IMU_CONFIG_RETRY;retry++){
    	...
    	else if(retry==(IMU_CONFIG_RETRY-1)) return 0x03;
    }
	...
	for(uint32_t retry=0;retry<IMU_CONFIG_RETRY;retry++){
		if(imuAckTransaction(IMUhandle,&cmd,&ack,IMU_ACK_DELAY)) break;
		else if(retry==(IMU_CONFIG_RETRY-1)) return 0x04;
	}

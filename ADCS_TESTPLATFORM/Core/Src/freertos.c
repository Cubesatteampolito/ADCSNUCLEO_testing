	...
	for (;;)
	{
		...
		if (rxLen)
		{
			...
			if (rxBuff[0] == SETOPMODEADCS_CODE && rxLen == sizeof(setOpmodeADCS))
			{
				...
				{
#if enable_printf
					printf("OBC TASK: allocazione struttura RxOpMode fallita !\n");
#endif
				}
				else
				{
...
//#endif
					...
					{
#if enable_printf
						printf("Dati OpMode Inviati a Control Task\n");
#endif
					}
				}
...
#if enable_printf
					printf("Desired P parameter in x: %f \n", RxAttitude->P_xx);
					printf("Desired P parameter in y: %f \n", RxAttitude->P_yy);
					printf("Desired P parameter in z: %f \n", RxAttitude->P_zz);
					printf("Desired D parameter in x: %f \n", RxAttitude->D_xx);
					printf("Desired D parameter in y: %f \n", RxAttitude->D_yy);
					printf("Desired D parameter in z: %f \n", RxAttitude->D_zz);
					printf("Desired Magn Field in x: %f \n", RxAttitude->db_x);
					printf("Desired Magn Field in y: %f \n", RxAttitude->db_y);
					printf("Desired Magn Field in z: %f \n", RxAttitude->db_z);
					printf("Desired Ang Speed Field in x: %f \n", RxAttitude->dtheta_x);
					...
					if(retvalue1.status == osEventMessage && retvalue2.status == osEventMessage)
					{
						if(!PID_Inputs.angSpeed_Desired[0] && !PID_Inputs.angSpeed_Desired[1] && !PID_Inputs.angSpeed_Desired[2])
						{
							...
							// {
								...
								// Change dir :
								// update_duty_dir(&MagneTorquer1,50,1);

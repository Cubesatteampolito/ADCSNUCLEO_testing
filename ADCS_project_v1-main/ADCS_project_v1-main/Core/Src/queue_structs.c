...
void receive_OpModequeue_control(void *event,void *opmode)
{
	imu_queue_struct *int_queue_struct;
	uint8_t *int_opmode = (uint8_t *)opmode;
...
void receive_Attitudequeue_control(void *event,void * PID_struct)
{
	...
	{	
		...
		//Inserire campo magnetico desiderato WAY-THOMPSON
		...
		for(int i=0;i<3;i++)
		{
			...
			printf("Control Task: int_PID_struct->P_Gain[%d]: %f \n",i,int_PID_struct->P_Gain[i]);
#endif
		}
		for(int i=0;i<3;i++)
		{
			...
			printf("Control Task: int_PID_struct->D_Gain[%d]: %f \n",i,int_PID_struct->D_Gain[i]);
#endif
		}*/
	}
	else{
